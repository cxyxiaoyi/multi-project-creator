...
class FormView {
    ...
    async show() {
        ...
        this.panel = vscode.window.createWebviewPanel('multiProjectCreatorForm', 'Multi-Project Creator', vscode.ViewColumn.One, {
            enableScripts: true,
...
<body>
  ...
  <script>
    ...
    async handleSubmit(data) {
        const errors = validator_1.Validator.validateForm(data);
        if (errors.length > 0) {
        ...
        try {
            const result = await projectService_1.ProjectService.createProject(data, (progressMessage, progressValue) => {
                this.panel?.webview.postMessage({
                    type: 'progress',
