...
<body>
    ...
    switch (message.type) {
      ...
      case 'addTemplate':
        try {
          ...
          ProjectTemplateService.addTemplate(name, gitUrl);
          this.panel?.webview.postMessage({
            type: 'templateAdded',
            message: `已添加工程: ${name}`,
            logType: 'success'
      ...
      case 'updateTemplate':
        try {
          ...
          ProjectTemplateService.updateTemplate(id, { name, gitUrl });
          this.panel?.webview.postMessage({
  ...
  private async handleCreateProject(formData: any): Promise<void> {
    try {
      const data: FormData = {
      ...
      const result = await ProjectService.createProject(data, (msg, progress) => {
        this.panel?.webview.postMessage({
          type: 'log',
          message: msg,
          logType: 'progress'
...
export async function createProjectCommand(context: vscode.ExtensionContext): Promise<void> {
  ProjectTemplateService.setContext(context);
...
export async function openProjectByIdCommand(projectId: string): Promise<void> {
  const openResult = await ProjectService.openProject(projectId);
...
export async function refreshProjectsTreeView(): Promise<void> {
  ...
  if (selected) {
    ...
    if (action) {
      switch (action.action) {
        case 'open':
          const openResult = await ProjectService.openProject(selected.project.id);
          if (!openResult.success) {
            vscode.window.showErrorMessage(openResult.message);
          }
          break;
        ...
        case 'refresh':
          const refreshResult = await ProjectService.refreshProject(selected.project.id);
        ...
        case 'delete':
          const deleteResult = await ProjectService.deleteProjectFiles(selected.project.id, false);
        ...
        case 'deleteAll':
          const confirm = await vscode.window.showWarningMessage(
            `Are you sure you want to delete "${selected.project.name}" and all its files?`,
            { modal: true },
            'Delete',
            'Cancel'
          );
          ...
          if (confirm === 'Delete') {
            const deleteAllResult = await ProjectService.deleteProjectFiles(selected.project.id, true);
...
export function registerCommands(context: vscode.ExtensionContext): void {
  ...
  const createCommand = vscode.commands.registerCommand(
    'multiProjectCreator.createProject',
    () => createProjectCommand(context)
  );

  const listCommand = vscode.commands.registerCommand(
    'multiProjectCreator.listProjects',
    () => listProjectsCommand()
  );

  const openProjectByIdHandler = vscode.commands.registerCommand(
    'multiProjectCreator.openProjectById',
    (projectId: string) => openProjectByIdCommand(projectId)
  );

  // Register tree view
  projectTreeDataProvider = new ProjectTreeDataProvider();
  ...
  ProjectService.setOnProjectChangedCallback(refreshProjectsTreeView);
